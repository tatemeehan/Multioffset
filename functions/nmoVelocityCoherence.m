function [S,tau,v] = nmoVelocityCoherence(d,dt,h,vmin,vmax,nv,R,L,stretchMax)
% VelocityCoherence: A program to compute velocity spectra
%                    applying hyperbolic move out equation.
%
%  [S,tau,v] = nmoVelocityCoherence(d,dt,h,vmin,vmav,nv,R,L);
%
%  IN   data:      data
...
S = zeros(ntau,nv);         % Allocate Coherence Matrix

    for iv = 1:nv
        tmpData = d;
        for it = 1:ntau
           
            % Apply Normal MoveOut Forward Model
